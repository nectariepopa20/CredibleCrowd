...
final class MojangNameResolver {
 ...
 CompletableFuture<String> correctCase(String input){if(!input.matches("[A-Za-z0-9_]{3,16}"))return CompletableFuture.completedFuture(input);String key=input.toLowerCase(Locale.ROOT);Cache hit=cache.get(key);if(hit!=null&&hit.expiresAt>System.currentTimeMillis())return CompletableFuture.completedFuture(hit.name);HttpRequest request=HttpRequest.newBuilder(URI.create("https://api.minecraftservices.com/minecraft/profile/lookup/name/"+input)).timeout(Duration.ofSeconds(5)).header("User-Agent","CredibleCrowd/1.3.1").GET().build();return client.sendAsync(request,HttpResponse.BodyHandlers.ofString()).handle((response,error)->{String resolved=input;if(error==null&&response.statusCode()==200){Matcher matcher=NAME.matcher(response.body());if(matcher.find())resolved=matcher.group(1);}cache.put(key,new Cache(resolved,System.currentTimeMillis()+Duration.ofHours(6).toMillis()));return resolved;});}
