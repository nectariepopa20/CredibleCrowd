...
1. Put `CredibleCrowd-Velocity-1.4.5.jar` in Velocity's `plugins/` directory.
...
5. Put `CredibleCrowd-Paper-1.4.5.jar` on every Paper backend whose placeholders, tablist and `/list` output should be synchronized.
...
Install Citizens, `CredibleCrowd-Paper-1.4.5.jar`, and `CredibleCrowd-Citizens-1.4.5.jar` on a Paper backend to materialize a bounded subset of its virtual population as player NPCs. The module requires both plugins and disables itself cleanly if either is unavailable.
